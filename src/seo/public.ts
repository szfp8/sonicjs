...
async function saveStoredSettings(db: D1Database, category: string, incoming: Record<string, any>): Promise<boolean> {
  try {
    ...
    ).bind(SETTINGS_TYPE, SETTINGS_TYPE, 'Site Settings', 'Global site configuration', '{}', 'system', 1, 1, now, now).run();
    const row = await db.prepare(
    ...
    if (row?.id) {
      ...
      ).bind(id, id, SETTINGS_TYPE, category, category, SETTINGS_TENANT, jsonData, now, now).run();
    }
    return true;
  } catch (e) {
    console.error('[tax-seo-settings] save failed', e);
    return false;
  }
}

/** 前台读取：后台「首页设置」+「获客设置」合并 */
...
/** 后台「SEO城市页面」集合 name = seo_city_page；type_id 可能是 name 或 UUID */
async function loadCityDocument(db: D1Database, cityName: string) {
  const parseRow = (row: { title?: string; data?: string } | null) => {
  ...
  try {
    ...
    // 2) type_id 为 document_types 表中 name=seo_city_page 的 id
    row = await db.prepare(`
      SELECT d.title, d.data FROM documents d
      JOIN document_types t ON t.id = d.type_id
      WHERE d.tenant_id = 'default' AND t.name = 'seo_city_page'
        AND d.is_published = 1 AND d.deleted_at IS NULL
        AND (json_extract(d.data, '$.city') = ? OR d.title LIKE ?)
  ...
  // 3) 无 json_extract：扫描已发布城市页
  try {
    const rows = await db.prepare(`
      SELECT title, data FROM documents
      WHERE tenant_id = 'default' AND is_published = 1 AND deleted_at IS NULL
        AND (type_id = 'seo_city_page' OR type_id IN (SELECT id FROM document_types WHERE name = 'seo_city_page'))
      ORDER BY updated_at DESC LIMIT 300
    `).all<{ title?: string; data?: string }>();
    for (const row of rows.results || []) {
      const parsed = parseRow(row);
      if (parsed && (parsed.city === cityName || (row.title || '').includes(cityName))) return parsed;
    }
  } catch {
    /* ignore */
  }
...
<header class="site-header"><div class="wrap header-inner">
  ...
  <nav class="nav">
    ...
    <a href="/contact${q}">${esc(home.navContact || m.navAbout)}</a>
  </nav>
  <div class="header-actions">
    <form class="header-search" action="/search" method="get"><input name="q" placeholder="${esc(home.navSearch || m.searchPlaceholder)}" /><button type="submit" aria-label="search">🔍</button></form>
...
// —— 后台设置 API（与 admin-beginner-ui 中 /admin/settings/api/* 对应）——
adminSettingsRoutes.get('/api/seo', async (c) => {
  const s = await getStoredSettings(c.env.DB, 'seo');
  return c.json({
    success: true,
    data: {
      seoTitle: s.seoTitle || '专业财税服务｜代理记账 代开发票 税务申报',
      seoKeywords: s.seoKeywords || '代理记账,代开发票,税务申报,财税咨询',
      seoDescription: s.seoDescription || '专业财税服务，助力企业发展。',
...
async function home(request: Request, env: SeoEnv, lang: LanguageCode) {
  ...
  const showAllCities = url.searchParams.get('cities') === 'all';

  const featured = showAllCities
    ? SEO_CITIES.map((c) => c.name)
    : ['北京', '上海', '广州', '深圳', '杭州', '成都', '重庆', '南京', '苏州', '天津'];
  const cityChips = featured.map((name) => `<a href="${cityUrl(request, name)}">${esc(name)}</a>`).join('');
  ...
  const newsPreview =
    lang === 'zh'
      ? [
          ['国家税务总局：优化营商环境，进一步便利纳税', '政策解读', '2025-05-20'],
          ['小微企业所得税优惠政策最新解读', '税收政策', '2025-05-18'],
          ['电子发票全面推广，企业如何应对？', '发票知识', '2025-05-16'],
        ]
      : [
          ['Tax authority updates', 'Policy', '2025-05-20'],
          ['SME tax incentives', 'Tax', '2025-05-18'],
          ['E-invoicing rollout', 'Invoice', '2025-05-16'],
        ];
  const newsItems = newsPreview
    .map(
      ([title, tag, date]) =>
        `<a class="news-item" href="/news${q}"><div class="thumb">📰</div><div><h3>${esc(title)}</h3><p>${esc(tag)} · ${esc(date)}</p></div></a>`,
    )
    .join('');

  // 后台首页设置 → 前台：标题 / 介绍 / 合规提示 / 模块开关
  const heroTitle = homeCfg.homeTitle || m.homeHeroTitle;
  const heroIntro = homeCfg.homeIntro || m.homeHeroBody;
  const heroSub = m.homeHeroSub;

  const citiesBlock = homeCfg.showCities
    ? `<section class="section" id="cities"><div class="section-head"><h2>${esc(m.homeCitiesTitle)}</h2><a href="${showAllCities ? '/#cities' : '/?cities=all#cities'}">${lang === 'zh' ? (showAllCities ? '收起' : '查看更多城市 →') : (showAllCities ? 'Less' : 'More →')}</a></div><div class="city-chips">${cityChips}</div></section>`
    : '';

  const newsBlock = homeCfg.showNewsBlock
    ? `<section class="section"><div class="section-head"><h2>${esc(m.homeNewsTitle)}</h2><a href="/news${q}">${esc(m.homeNewsBtn)}</a></div><div class="news-list">${newsItems}</div></section>`
    : '';
...
<section class="hero"><div class="hero-bg"></div><div class="hero-inner">
  ...
  <form class="hero-search" action="/search" method="get">
    <select name="city" aria-label="city"><option value="">${lang === 'zh' ? '城市' : 'City'}</option>${featured.slice(0, 10).map((c) => `<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select>
    <input name="q" placeholder="${esc(m.homeSearchPlaceholder)}" />
    <button type="submit">${esc(m.homeSearchBtn)}</button>
  </form>
</div></section>
<div class="svc-row">${svcRow}</div>
<section class="section"><div class="section-head"><h2>${esc(m.homeHotServices)}</h2><a href="/contact${q}">${lang === 'zh' ? '查看更多服务 →' : 'More →'}</a></div><div class="hot-grid">${hotCards}</div></section>
...
async function contactPage(request: Request, env: SeoEnv, lang: LanguageCode) {
  const m = t(lang);
  const homeCfg = await loadHomeSettings(env, lang);
  const body = `<section class="section"><h1>${esc(m.contactTitle)}</h1><p class="muted">${esc(m.contactDesc)}</p>
<form class="form" method="post" action="/api/lead">
<input name="name" placeholder="${esc(m.contactName)}" required>
<input name="phone" placeholder="${esc(m.contactPhone)}" required>
<input name="city" placeholder="${esc(m.contactCity)}">
<textarea name="need" rows="4" placeholder="${esc(m.contactNeed)}" required></textarea>
<button class="btn" type="submit">${esc(m.contactSubmit)}</button>
</form></section>`;
...
async function cityPage(request: Request, env: SeoEnv, city: string, lang: LanguageCode) {
  ...
  const content = doc?.contentHtml
    ...
    : `<div class="card"><p>${lang === 'zh' ? `${city}提供代理记账、代开发票、税务申报、财税咨询等服务信息。可在后台「城市管理」发布定制正文。` : `${city}: bookkeeping and tax consulting.`}</p><p class="muted">${esc(homeCfg.homeNotice || m.homeNotice)}</p></div>`;
  const body = `<section class="section"><h1>${esc(title)}</h1>${doc?.province ? `<p class="muted">${esc(doc.province)}</p>` : ''}${content}<p style="margin-top:16px"><a class="btn" href="/contact">${esc(m.homeContactBtn)}</a> <a class="btn" href="/#cities" style="background:#64748b;margin-left:8px">${esc(m.homeCitiesTitle)}</a></p></section>`;
  return page(title, desc, body, env, request, lang, homeCfg, cityUrl(request, city));
}
...
export async function handleSeoRequest(request: Request, env: SeoEnv): Promise<Response | null> {
  ...
  if (request.method === 'GET' && url.pathname.startsWith('/city/')) {
    const raw = url.pathname.slice('/city/'.length);
