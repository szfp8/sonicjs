...
async function saveStoredSettings(db: D1Database, category: string, incoming: Record<string, any>): Promise<boolean> {
  try {
    ...
    await db.prepare(`INSERT OR IGNORE INTO document_types (id, name, display_name, description, schema, source, is_system, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(SETTINGS_TYPE, SETTINGS_TYPE, 'Site Settings', 'Global', '{}', 'system', 1, 1, now, now).run();
...
async function loadCityDocument(db: D1Database, cityName: string) {
  const parse = (row: { title?: string; data?: string } | null) => {
    ...
    try {
      ...
      return { title: String(data.title || row.title || ''), metaDescription: String(data.metaDescription || ''), contentHtml: richText(data.content), province: String(data.province || ''), city: String(data.city || '') };
    } catch { return null; }
  };
  try {
    let row = await db.prepare(`SELECT title, data FROM documents WHERE tenant_id='default' AND type_id='seo_city_page' AND is_published=1 AND deleted_at IS NULL AND (json_extract(data,'$.city')=? OR title LIKE ?) ORDER BY updated_at DESC LIMIT 1`).bind(cityName, `%${cityName}%`).first<{ title?: string; data?: string }>();
    let p = parse(row); if (p) return p;
    row = await db.prepare(`SELECT d.title, d.data FROM documents d JOIN document_types t ON t.id=d.type_id WHERE d.tenant_id='default' AND t.name='seo_city_page' AND d.is_published=1 AND d.deleted_at IS NULL AND (json_extract(d.data,'$.city')=? OR d.title LIKE ?) ORDER BY d.updated_at DESC LIMIT 1`).bind(cityName, `%${cityName}%`).first<{ title?: string; data?: string }>();
    return parse(row);
  } catch { return null; }
}
...
async function page(title: string, description: string, body: string, env: SeoEnv, request: Request, lang: LanguageCode, home: HomeSettings, canonical?: string, activeNav?: string) {
  const m = t(lang);
...
adminSettingsRoutes.get('/api/seo', async (c) => {
  ...
  return c.json({ success: true, data: { seoTitle: s.seoTitle || '专业财税服务', seoKeywords: s.seoKeywords || '', seoDescription: s.seoDescription || '', canonicalUrl: s.canonicalUrl || '', robots: s.robots || 'index,follow', indexNowEnabled: s.indexNowEnabled !== false } });
});
...
adminSettingsRoutes.get('/api/lead', async (c) => {
  ...
  return c.json({ success: true, data: { contactPhone: s.contactPhone || '', wechat: s.wechat || '', qq: s.qq || '', telegram: s.telegram || '', whatsapp: s.whatsapp || '', leadEnabled: s.leadEnabled !== false, leadMessage: s.leadMessage || '请通过正规渠道提交真实业务需求。' } });
});
adminSettingsRoutes.post('/api/lead', async (c) => {
  const body = await c.req.json().catch(() => ({}));
  const ok = await saveStoredSettings(c.env.DB, 'lead', { contactPhone: String(body.contactPhone||'').slice(0,80), wechat: String(body.wechat||'').slice(0,120), qq: String(body.qq||'').slice(0,40), telegram: String(body.telegram||'').slice(0,120), whatsapp: String(body.whatsapp||'').slice(0,40), leadEnabled: body.leadEnabled !== false, leadMessage: String(body.leadMessage||'').slice(0,500) });
...
async function home(request: Request, env: SeoEnv, lang: LanguageCode) {
  ...
  const featured = showAll ? SEO_CITIES.map((c) => c.name) : ['北京','上海','广州','深圳','杭州','成都','重庆','南京','苏州','天津'];
  const cityChips = featured.map((n) => `<a href="${cityUrl(request, n)}">${esc(n)}</a>`).join('');
  const services = [{ icon: '📒', title: '代理记账', desc: '专业团队' },{ icon: '🧾', title: '发票服务', desc: '电子发票' },{ icon: '📑', title: '税务申报', desc: '按时申报' },{ icon: '💬', title: '财税咨询', desc: '一对一' },{ icon: '🏢', title: '工商服务', desc: '注册变更' }];
  ...
  const hot = [{ icon: '📊', title: '代理记账', desc: '专业会计' },{ icon: '⚡', title: '发票代开', desc: '快速办理' },{ icon: '📅', title: '税务申报', desc: '规避风险' },{ icon: '🤝', title: '财税咨询', desc: '量身定制' }];
  ...
  const citiesBlock = homeCfg.showCities ? `<section class="section" id="cities"><div class="section-head"><h2>${esc(m.homeCitiesTitle)}</h2><a href="${showAll ? '/#cities' : '/?cities=all#cities'}">${showAll ? '收起' : '查看更多 →'}</a></div><div class="city-chips">${cityChips}</div></section>` : '';
  const newsBlock = homeCfg.showNewsBlock ? `<section class="section"><div class="section-head"><h2>${esc(m.homeNewsTitle)}</h2><a href="/news${q}">${esc(m.homeNewsBtn)}</a></div><p class="muted">${esc(m.homeNewsBody)}</p></section>` : '';
  const wechatBlock = homeCfg.showWechatBlock ? `<section class="section"><div class="section-head"><h2>${esc(m.homeWechatTitle)}</h2><a href="/wechat${q}">${esc(m.homeWechatBtn)}</a></div><p class="muted">${esc(m.homeWechatBody)}</p></section>` : '';
  const body = `<section class="hero"><div class="hero-inner"><h1>${esc(homeCfg.homeTitle || m.homeHeroTitle)}</h1><p class="hero-sub">${esc(m.homeHeroSub)}</p><p class="hero-intro">${esc(homeCfg.homeIntro || m.homeHeroBody)}</p><form class="hero-search" action="/search" method="get"><select name="city"><option value="">城市</option>${featured.slice(0,10).map((c)=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select><input name="q" placeholder="${esc(m.homeSearchPlaceholder)}"/><button type="submit">${esc(m.homeSearchBtn)}</button></form></div></section><div class="svc-row">${svcRow}</div><section class="section"><div class="section-head"><h2>${esc(m.homeHotServices)}</h2><a href="/contact${q}">更多 →</a></div><div class="hot-grid">${hotCards}</div></section>${citiesBlock}${newsBlock}${wechatBlock}<div class="advisor"><div><h3>${esc(m.homeAdvisorTitle)}</h3><p>${esc(homeCfg.homeNotice || m.homeNotice)}</p></div><a class="btn btn-amber" href="/contact${q}">${esc(m.homeAdvisorBtn)}</a></div>`;
...
async function cityPage(request: Request, env: SeoEnv, city: string, lang: LanguageCode) {
  ...
  const content = doc?.contentHtml ? `<div class="card">${doc.contentHtml}</div>` : `<div class="card"><p>${city}提供代理记账、代开发票、税务申报、财税咨询等服务信息。</p><p class="muted">${esc(homeCfg.homeNotice || m.homeNotice)}</p></div>`;
  return await page(title, doc?.metaDescription || m.homeDesc, `<section class="section"><h1>${esc(title)}</h1>${content}<p style="margin-top:16px"><a class="btn" href="/contact">${esc(m.homeContactBtn)}</a></p></section>`, env, request, lang, homeCfg, cityUrl(request, city));
}
...
export async function pingIndexNow(request: Request, env: SeoEnv) {
  ...
  } catch (e) { console.warn('[indexnow]', e); }
}
