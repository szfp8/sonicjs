 ...
 * Presentation only: SonicJS continues to handle authentication, permissions,
 * content APIs and D1 persistence.
 */
...
(function () {
  ...
  function addStyles() {
    ...
    style.textContent =
      ...
      '@media(max-width:700px){.tax-seo-quick,.tax-seo-setting-grid{grid-template-columns:1fr}}';
  ...
  function buildSettingsGuide() {
    ...
    guide.innerHTML =
      '<h4>⚙ 财税SEO网站设置中心</h4>' +
      '<p>小白模式：先完成基础资料，再管理内容、城市SEO和收录。下面入口全部使用现有 SonicJS 功能，不需要改代码。</p>' +
      '<div class="tax-seo-setting-grid">' +
        '<div class="tax-seo-setting-card"><b>① 网站基础信息</b><span>网站名称、管理员邮箱、网站描述、时区和维护模式。</span><a href="#settings-content">当前页面设置 →</a></div>' +
        '<div class="tax-seo-setting-card"><b>② 内容管理</b><span>发布网站资讯、政策解读和 SEO 资讯。</span><a href="/admin/content">进入内容管理 →</a></div>' +
        '<div class="tax-seo-setting-card"><b>③ 全国城市SEO</b><span>管理全国城市落地页，让城市关键词有独立页面。</span><a href="/admin/content?model=seo_city_page">管理城市页面 →</a></div>' +
        '<div class="tax-seo-setting-card"><b>④ 政策解读</b><span>按照“来源 + 原创解读 + 企业实际价值”组织内容。</span><a href="/admin/content?model=seo_article">管理政策内容 →</a></div>' +
        '<div class="tax-seo-setting-card"><b>⑤ 收录工具</b><span>Sitemap、Robots 和 IndexNow 已由网站程序提供。</span><a href="/sitemap.xml" target="_blank">查看 Sitemap →</a></div>' +
        '<div class="tax-seo-setting-card"><b>⑥ 系统工具</b><span>媒体、日志、数据库工具等保留原 SonicJS 能力。</span><a href="/admin/settings/database-tools">进入系统工具 →</a></div>' +
      '</div>' +
      '<div class="tax-seo-status">✓ 当前版本不新增 D1 表、不执行迁移；设置仍使用现有 SonicJS 设置存储，尽量减少 D1 消耗。</div>';
  ...
  function buildHomePanel() {
    ...
    quick.innerHTML=menu.slice(1).map(function(item){
      var desc=item.text==='内容管理'?'查看和管理网站文章':item.text==='网站资讯'?'管理普通网站资讯':item.text==='政策解读 / SEO资讯'?'来源 + 原创解读 + 企业实际价值':item.text==='SEO城市页面'?'管理全国城市SEO落地页':item.text==='网站用户'?'管理后台账号和权限':'管理网站基础信息';
  ...
  function buildPageShell() {
    ...
    addStyles(); buildQuickMenu(); buildSettingsGuide(); buildHomePanel();
